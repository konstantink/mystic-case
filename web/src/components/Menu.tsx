...
interface MenuItemProps extends React.PropsWithChildren<_MenuItemProps> {}

type MenuItemName = 'home' | 'shop' | 'contacts' | 'log in' | 'register' | 'cart';


const MenuItem = ({ children, to, timeout }: MenuItemProps) => {
...
const FacebookLogo = ({ timeout }: LogoProps) => {
...
const InstagramLogo = ({ timeout }: LogoProps) => {
    const classes = logoStyles();
    const transitions = useTransition(true, {
        from: { transform: "scale(0)" },
        enter: { transform: "scale(1)", delay: timeout },
...
const Menu = ({ classes }: MenuProps) => {
    ...
    return (
        <Box component="div" className={classes.root}>
            ...
            <FacebookLogo timeout={(menuItems.length+1)*150}/>
            <InstagramLogo timeout={(menuItems.length+2)*150} />
        </Box>
    )
}
